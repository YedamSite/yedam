...
export default function BrandsTab() {
  ...
  return (
    <div className="bg-card border border-white/5 rounded-3xl p-6 md:p-8 shadow-xl">
      <h2 className="font-heading text-2xl font-light text-white uppercase tracking-wide border-b border-white/5 pb-4 mb-6">{t('Gerenciar Marcas')}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-secondary/30 border border-white/5 rounded-2xl p-6 h-fit text-xs text-muted-foreground space-y-4">
          <h3 className="text-xs font-bold text-accent uppercase tracking-wider">{editingId ? t('Editar Marca') : t('Nova Marca')}</h3>
          ...
          <form onSubmit={handleSave} className="flex flex-col gap-4">
            {activeLang === 'es' ? (
              <>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase text-accent">{t('Nome (ES)')}</label>
                  <Input required value={formName} onChange={e => { setFormName(e.target.value); if (!editingId) setFormSlug(e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '-')); }} placeholder={t('Ex: Round Lab')} />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase text-accent">{t('Slug (URL)')}</label>
                  <Input value={formSlug} onChange={e => setFormSlug(e.target.value)} placeholder={t('round-lab')} />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase text-accent">{t('Descrição (ES)')}</label>
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  <label htmlFor="featured" className="text-[10px] font-bold text-accent uppercase select-none cursor-pointer">{t('Marca em Destaque')}</label>
                </div>
                <ImageUpload
                  currentUrl={formLogo}
                  onUrlChange={setFormLogo}
                  folder="brands"
                  label={t('Logo da Marca')}
                />
              </>
            ) : (
              <>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase text-accent">{t('Nome')} ({activeLang.toUpperCase()})</label>
                  <Input 
                    value={translations[activeLang].name} 
                    onChange={e => updateTranslationField('name', e.target.value)} 
                    placeholder={t('[ES]:') + ' ' + formName} 
                  />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase text-accent">{t('Descrição')} ({activeLang.toUpperCase()})</label>
                  <textarea 
                    value={translations[activeLang].description} 
                    onChange={e => updateTranslationField('description', e.target.value)} 
                    placeholder={t('[ES]:') + ' ' + formDesc} 
                    rows={3} 
            ...
            <div className="flex gap-2 pt-2">
              <Button type="submit" className="bg-accent hover:bg-accentHover text-background font-bold py-2 rounded-xl text-xs flex-1">
                {editingId ? t('ATUALIZAR') : t('CRIAR')}
              </Button>
              {editingId && (
                <Button type="button" onClick={resetForm} variant="outline" className="border-white/10 text-white py-2 rounded-xl text-xs">{t('CANCELAR')}</Button>
              )}
            </div>
          </form>
        </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          <h3 className="text-xs font-bold text-white uppercase tracking-wider mb-2">{t('Marcas')} ({brands.length})</h3>
          {brands.map((brand) => (
            <div key={brand.id} className="border border-white/5 rounded-xl p-4 bg-secondary/30 flex items-center justify-between gap-4 text-xs">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {brand.is_featured && <span className="text-[8px] text-accent bg-accent/10 border border-accent/20 px-1.5 py-0.5 rounded-full uppercase font-bold">{t('Destaque')}</span>}
                </div>
