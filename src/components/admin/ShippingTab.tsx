  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
        ...
        <Button onClick={saveZones} disabled={loading} className="bg-accent hover:bg-accentHover text-background font-bold text-xs flex gap-2 self-start sm:self-auto">
          ...
          <div key={zIdx} className="border border-white/10 rounded-2xl p-5 bg-card flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              ...
              {zone.methods.map((method: any, mIdx: number) => (
                <div key={mIdx} className="flex flex-wrap sm:flex-nowrap items-center gap-3 bg-secondary/30 p-3 rounded-xl border border-white/5">
                  <Input 
                    value={method.name} 
                    onChange={(e) => updateMethod(zIdx, mIdx, 'name', e.target.value)}
                    placeholder="Nome (ex: K-Packet)"
                    className="bg-black/30 border-white/10 text-xs h-8 flex-1 min-w-[120px]"
                  />
                  <div className="flex items-center gap-2 shrink-0">
                    <Input 
                      ...
                      className="bg-black/30 border-white/10 text-xs h-8 w-20 text-center"
                    />
                  ...
                  <Button onClick={() => removeMethod(zIdx, mIdx)} variant="ghost" className="text-red-400 hover:text-red-300 p-1.5 h-8 shrink-0 ml-2">
