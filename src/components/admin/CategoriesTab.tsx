...
export default function CategoriesTab() {
  ...
  return (
    <div className="bg-card border border-white/5 rounded-3xl p-6 md:p-8 shadow-xl">
      <h2 className="font-heading text-2xl font-light text-white uppercase tracking-wide border-b border-white/5 pb-4 mb-6">{t('Gerenciar Categorias')}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-secondary/30 border border-white/5 rounded-2xl p-6 h-fit text-xs text-muted-foreground space-y-4">
          <h3 className="text-xs font-bold text-accent uppercase tracking-wider">{editingId ? t('Editar Categoria') : t('Nova Categoria')}</h3>
            ...
            {activeLang === 'es' ? (
              <>
                <div className="flex flex-col gap-1.5">
                  ...
                  <Input required value={formName} onChange={e => { setFormName(e.target.value); if (!editingId) setFormSlug(generateSlug(e.target.value)); }} placeholder={t('Ex: Cuidado Facial')} />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold uppercase text-accent">{t('Slug (URL)')}</label>
                  <Input value={formSlug} onChange={e => setFormSlug(e.target.value)} placeholder={t('cuidado-facial')} />
                </div>
                <div className="flex flex-col gap-1.5">
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          <h3 className="text-xs font-bold text-white uppercase tracking-wider mb-2">{t('Categorias')} ({categories.length})</h3>
          {categories.map((cat) => (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-white">
                    {cat.name} 
                    {cat.translations?.pt?.name && <span className="text-[8px] text-green-400 border border-green-500/20 px-1 rounded ml-2 font-normal">{t('PT')}</span>}
                    {cat.translations?.en?.name && <span className="text-[8px] text-blue-400 border border-blue-500/20 px-1 rounded ml-1 font-normal">{t('EN')}</span>}
                  </h4>
